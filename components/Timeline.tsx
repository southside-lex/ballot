...
export default function Timeline({ elections }: { elections: Election[] }) {
  return (
    <section className="py-20 border-t border-border">
      ...
      {elections.length === 0 ? (
        <p className="text-text-muted italic text-sm">No upcoming elections.</p>
      ) : (
        <ol className="divide-y divide-border border-y border-border">
          {elections.map((e) => (
            ...
            >
              <time className="font-mono text-sm text-brand tabular-nums whitespace-nowrap w-28 shrink-0">
                {e.date
              ...
              <div className="flex-1">
                ...
                {e.type && (
                  <p className="text-xs font-mono uppercase tracking-wider text-text-muted mt-0.5 capitalize">
                    {e.type}
                  </p>
                )}
              </div>
